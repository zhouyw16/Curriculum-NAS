...
def search_func(xloader, network, w_criterion, criterion, scheduler, 
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets, indices) in enumerate(xloader):
        ...
        network.zero_grad()
        if algo == "darts-v2":
            arch_loss, logits = backward_step_unrolled(network, criterion,
...
def train_controller(xloader, network, criterion, optimizer, 
    prev_baseline, epoch_str, print_freq, logger):
    # config. (containing some necessary arg)
    #   baseline: The baseline score (i.e. average val_acc) from the previous epoch
    data_time, batch_time = AverageMeter(), AverageMeter()
    (GradnormMeter, LossMeter, ValAccMeter, EntropyMeter, BaselineMeter, RewardMeter, xend, ) \
    = (AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), time.time(),)
    ...
    for step in range(controller_train_steps * controller_num_aggregate):
        ...
        if step % print_freq == 0:
            Sstr = ("*Train-Controller* " + time_string() + " [{:}][{:03d}/{:03d}]".format(
...
def main(xargs):
    ...
    model_config = dict2config(
        dict(name="generic",  # so it takes GenericNAS201Model
            C=xargs.channel, N=xargs.num_cells,
            max_nodes=xargs.max_nodes, num_classes=class_num,
            space=search_space, affine=bool(xargs.affine),
    ...
    a_optimizer = torch.optim.Adam(search_model.alphas,
        lr=xargs.arch_learning_rate, betas=(0.5, 0.999),
        weight_decay=xargs.arch_weight_decay, eps=xargs.arch_eps,)
    ...
    last_info, model_base_path, model_best_path = (
        logger.path("info"), logger.path("model"), logger.path("best"),)
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        logger.log("[{:}] search [arch] : loss={:.2f}, accuracy@1={:.2f}%, accuracy@5={:.2f}%".format(
        ...
        if xargs.algo == "enas":
            ...
            logger.log("[{:}] controller : loss={:}, acc={:}, baseline={:}, reward={:}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--search_space", type=str, default="tss", choices=["tss"],
